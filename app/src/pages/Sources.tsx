...
export default function Sources() {
  return (
    <div className="mx-auto max-w-6xl px-4 py-12 sm:py-16">
      ...
      <div className="mt-8 overflow-x-auto rounded-xl border border-stone-200 dark:border-stone-800">
        <table className="w-full text-sm">
          ...
          <tbody>
            {sources.map((s, i) => (
              <tr key={s.url} className="border-b border-stone-100 dark:border-stone-800 last:border-0 hover:bg-stone-50 dark:hover:bg-stone-900">
                <td className="px-4 py-3 tabular-nums text-stone-500 dark:text-stone-400">{i + 1}</td>
                <td className="px-4 py-3">
                  <a
                    href={s.url}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-teal-700 dark:text-teal-400 hover:underline"
                  >
                    {s.name} <ExternalLink className="h-3.5 w-3.5" />
                  </a>
                </td>
                <td className="hidden px-4 py-3 text-stone-600 dark:text-stone-300 md:table-cell">{s.used}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {/* Archived documents */}
      <div className="mt-8 rounded-xl border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-900 p-5 text-sm text-stone-600 dark:text-stone-300">
        <h3 className="mb-1 font-display font-semibold tracking-tight text-stone-800 dark:text-stone-200">Archived documents</h3>
      ...
      />
      <p className="mt-6 max-w-prose text-sm text-stone-600 dark:text-stone-300">
        Guide photos, topos and hand-drawn maps (paths <code className="text-teal-700 dark:text-teal-400">images/guide/*</code>)
        are extracted from the{' '}
        <a href={GUIDE_PDF_URL} target="_blank" rel="noreferrer" className="text-teal-700 dark:text-teal-400 hover:underline">
        ...
        <code className="text-teal-700 dark:text-teal-400">images/community/*</code>) are © their authors and used
        under the licenses listed below — CC-ND images are shown unmodified.
      </p>

      <p className="mb-2 mt-6 text-xs text-stone-500 dark:text-stone-400 md:hidden">Swipe sideways to see the full table →</p>
      <div className="overflow-x-auto rounded-xl border border-stone-200 dark:border-stone-800 md:mt-0">
        <table className="w-full min-w-[560px] text-sm">
          ...
          <tbody>
            {communityPhotos.map((p) => (
              <tr key={p.file} className="border-b border-stone-100 dark:border-stone-800 last:border-0 hover:bg-stone-50 dark:hover:bg-stone-900">
                <td className="px-4 py-3 font-mono text-xs text-stone-600 dark:text-stone-300">
                  {p.file.replace('images/community/', '')}
                </td>
                <td className="hidden max-w-xs px-4 py-3 text-xs text-stone-600 dark:text-stone-300 lg:table-cell">
                  <span className="line-clamp-2">{p.caption}</span>
                </td>
                <td className="px-4 py-3 text-stone-800 dark:text-stone-200">{p.credit}</td>
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full border px-2 py-0.5 text-xs whitespace-nowrap ${
                      isNdLicense(p)
                        ? 'border-amber-300 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/15 text-amber-800 dark:text-amber-300'
      ...
      <div className="mt-8 rounded-xl border border-stone-200 dark:border-stone-800 bg-stone-50 dark:bg-stone-900 p-5 text-sm text-stone-600 dark:text-stone-300">
        <h3 className="mb-1 font-display font-semibold tracking-tight text-stone-800 dark:text-stone-200">Obsidian mirror</h3>
        This site is generated to mirror the Obsidian vault in <code className="text-teal-700 dark:text-teal-400">/vault</code> —
