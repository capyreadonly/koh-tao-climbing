...
Updated 30 Aug 2026 (Asia/Bangkok). Living backlog for the native iOS app. Highest first.

Skip anything already in the 0.1 (3) review binary: About + first-run, privacy/support pages, map QA (callouts, legend, FABs, unmapped badge, clustering, VoiceOver, coverage banner), show-on-map, launch screen, PrivacyInfo. TestFlight 0.1 (2) is superseded. App Store review is a separate watch — do not archive or upload from this list.

## Next

1. **Merge [PR #3](https://github.com/capyreadonly/koh-tao-climbing/pull/3)** so `main` matches the review binary (13 commits ahead of `main`, mergeable). Close superseded [#1](https://github.com/capyreadonly/koh-tao-climbing/pull/1) and [#2](https://github.com/capyreadonly/koh-tao-climbing/pull/2) after. PR title still says “build 2”; HEAD is build 3 (`CURRENT_PROJECT_VERSION` 3).
2. **Offline favorites / personal ticks.** Search exists; AppStorage is only map camera + first-run About. Climbers still cannot mark what they did. This is the next product ship after `main` is current.
